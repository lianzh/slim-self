...
class Arrays
{
     ...
     * $output = \SelfApp\Helper\Arrays::normalize($input);
     * // $output 现在是一个数组，结果如下：
     * // $output = [
     * //   'item1',
     * //   'item2',
     ...
     * @code php
     * $arr = ['', 'test', '   '];
     * \SelfApp\Helper\Arrays::remove_empty($arr);
     *
     * \Ws\Env::dump($arr);
     *   // 输出结果中将只有 'test'
     * @endcode
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $values = \SelfApp\Helper\Arrays::cols($rows, 'value');
     *
     * \Ws\Env::dump($values);
     *   // 输出结果为
     *   // [
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $hashmap = \SelfApp\Helper\Arrays::hashmap($rows, 'id', 'value');
     *
     * \Ws\Env::dump($hashmap);
     *   // 输出结果为
     *   // [
     ...
     *     ['id' => 2, 'value' => '2-1'],
     * ];
     * $hashmap = \SelfApp\Helper\Arrays::hashmap($rows, 'id');
     *
     * \Ws\Env::dump($hashmap);
     *   // 输出结果为
     *   // [
     ...
     * ];
     * $values = \SelfApp\Helper\Arrays::group_by($rows, 'parent');
     *
     * \Ws\Env::dump($values);
     *   // 按照 parent 分组的输出结果为
     *   // array(
     ...
     * ];
     *
     * $tree = \SelfApp\Helper\Arrays::tree($rows, 'id', 'parent', 'nodes');
     *
     * \Ws\Env::dump($tree);
     *   // 输出结果为：
     *   // [
     ...
     * @code php
     * $refs = null;
     * $tree = \SelfApp\Helper\Arrays::tree($rows, 'id', 'parent', 'nodes', $refs);
     * 
     ...
     * ];
     *
     * $rows = \SelfApp\Helper\Arrays::sort_by_col($rows, 'id', SORT_DESC);
     * \Ws\Env::dump($rows);
     * // 输出结果为：
     * // [
     * //   ['id' => 6, 'value' => '6-1', 'parent' => 3],
     ...
     * 用法：
     * @code php
     * $rows = \SelfApp\Helper\Arrays::sort_by_cols($rows, [
     *     'parent' => SORT_ASC, 
     *     'name' => SORT_DESC,
     * ]);
     * @endcode
