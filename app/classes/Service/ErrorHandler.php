...
class ErrorHandler
{
    ...
    {
        # 设置错误^异常处理函数
        error_reporting(Arrays::val($config, 'level', E_ALL | E_STRICT));
	...
	private static function formatErrno($errno)
	{
		...
		return Arrays::val($codes, $errno, 'Unknown error type');
	}
	...
	{
        if ( PHP_SAPI === 'cli' ) return self::consoleException($ex);

		$viewDir = __DIR__ . "/_errorViews";
	...
	{
        if ( PHP_SAPI === 'cli' ) return self::consoleFatal($error);
	...
	{
        if ( PHP_SAPI === 'cli' ) return self::consoleUserlevel($errno, $errstr, $errfile, $errline, $errcontext);
	...
	static function consoleException(\Exception $ex)
	{
		...
		CLI::write("[异常] {$exclass} (code={$code})",'red');
		CLI::write("[描述] {$msg}",'yellow');
		CLI::write("[{$line}] :{$file}");
		...
		foreach ($trace as $point) {
	        ...
	        CLI::write("[{$line}] :{$file}");
			CLI::write("{$function}($args)",'light_red');
			if ($dump_args)
			{
				CLI::write("[参数] " . print_r($point['args'],true) ,'light_red');
			}			
		}
	...
	{		
		...
		if (!empty($error))
		{			
			CLI::write("[错误] " . self::formatErrno($error['type']) . " `{$error['message']}` ",'light_red');
			CLI::write("[{$error['line']}] :{$error['file']}");
		}
		exit;
	}
	...
	{		
	    ...
	    {
	    	CLI::write("[提示] " . self::formatErrno($errno) . " `{$errstr}` ",'yellow');
	    	CLI::write("[{$errline}] :{$errfile}");
	    }
